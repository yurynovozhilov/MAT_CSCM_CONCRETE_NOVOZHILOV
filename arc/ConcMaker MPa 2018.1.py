...
def CDPM(
    ...
    damage_type = 'lin', # Flag for damage type: lin, bi, exp, no
...
rho  = 2.4E-9
rev  = 2
mat = keyword2text(CSCM(concreteClass=fc,dmax=dmax,irate='on',erode='off', pred='off', recov = 'full', rev=rev)) + printCEBFIP(fc, dmax, rho )
#mat = keyword2text(CDPM(concreteClass=fc,dmax=dmax,mesh='hex',strflag = 'on',damage_type = 'lin')) + printCEBFIP(fc, dmax, rho )
